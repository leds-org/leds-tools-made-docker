...
`;
  ...
  async generateFiles(outputDir, project) {
    try {
      ...
      const svgPath = import_path.default.join(outputDir, "project-cfd.svg");
      const projectCFD = new ProjectCFD(this.sprints, svgPath);
      projectCFD.generate();
      const svgPathTP = import_path.default.join(outputDir, "project-throughput.svg");
      const throughput = new ProjectThroughputGenerator(this.sprints, svgPathTP);
      throughput.generate();
      const markdown = this.generateMarkdownReport(project);
      const markdownPath = import_path.default.join(outputDir, "01_overview.md");
      await import_fs.default.promises.writeFile(markdownPath, markdown, "utf-8");
    } catch (error) {
...
var MarkdownBacklogService = class {
  ...
  async retrive(database) {
    ...
    const adapter = new import_node.JSONFileSync(ISSUEPATH);
    const defaultData = { data: [] };
    const db = new import_lowdb.LowSync(adapter, defaultData);
    await db.read();
...
var import_lowdb2 = require("lowdb");
var import_node2 = require("lowdb/node");
...
var MarkdownTimeBoxService = class {
  ...
  fileContainsName(directory, partialName) {
    try {
      const files = (0, import_fs2.readdirSync)(directory);
  ...
  async retrive(database) {
    ...
    const adapter = new import_node2.JSONFileSync(ISSUEPATH);
    const defaultData = { data: [] };
    const db = new import_lowdb2.LowSync(adapter, defaultData);
    await db.read();
    return db.data.data.sort((a, b) => {
      return Number(a.id) - Number(b.id);
    });
...
var import_lowdb3 = require("lowdb");
var import_node3 = require("lowdb/node");
...
var MardownRoadmapService = class {
  ...
  async retrive(database) {
    ...
    const adapter = new import_node3.JSONFileSync(ISSUEPATH);
    const defaultData = { data: [] };
    const db = new import_lowdb3.LowSync(adapter, defaultData);
    await db.read();
    return db.data.data.sort((a, b) => {
      return Number(a.id) - Number(b.id);
    });
